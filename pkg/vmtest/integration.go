...
type Options struct {
	...
	//
	// QEMUTest generates a JSON file with these commands. The file is shared
	// with the VM, and the commands are run from the generic uinit.
	TestCmds []string
...
func QEMU(o *Options) (*qemu.Options, error) {
	...
	// Generate JSON file of test commands in o.TmpDir.
	if len(o.TestCmds) > 0 {
		testFile := filepath.Join(o.TmpDir, "test.json")

		jsonCmds, err := json.Marshal(o.TestCmds)
		if err != nil {
			return nil, err
		}

		if err := ioutil.WriteFile(testFile, jsonCmds, 0755); err != nil {
			return nil, err
		}
	}
